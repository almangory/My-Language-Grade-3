  ...
  return (
    <div className="min-h-screen bg-cream pb-12 flex flex-col font-sans antialiased text-right text-charcoal" dir="rtl">
      ...
      <main className={`${mainTab === 'flipbook' ? 'max-w-none w-full px-2 md:px-6' : 'max-w-6xl w-full mx-auto px-4 md:px-8'} mt-6 flex-1 flex flex-col gap-6`}>
        ...
        {/* Offline & Connection Management Panel */}
        <div className="w-full max-w-4xl mx-auto bg-white p-4 rounded-3xl border-2 border-yellow-border shadow-sm flex flex-col md:flex-row items-center justify-between gap-4 select-none">
          {/* Connection Status Badge */}
          <div className="flex items-center gap-3">
            <div className={`w-10 h-10 rounded-full flex items-center justify-center transition-all ${
              isOnline ? 'bg-emerald-100 text-emerald-600' : 'bg-amber-100 text-amber-600 animate-pulse'
            }`}>
              {isOnline ? <Wifi className="w-5 h-5" /> : <WifiOff className="w-5 h-5" />}
            </div>
            <div className="text-right">
              <span className="text-xs font-black text-slate-500 block leading-none">حالة الاتصال</span>
              <span className={`text-sm font-black ${isOnline ? 'text-emerald-600' : 'text-amber-600 animate-pulse'}`}>
